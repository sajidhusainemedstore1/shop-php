...
<body>
    ...
    <?php if (isset($order['return_status'])): ?>
        <?php if ($order['return_status'] === 'approved'): ?>
            ...
            <?php if (!empty($order['return_approve_comment'])): ?>
                <div style="margin-top: 5px;">Approve Comment: <?= htmlspecialchars($order['return_approve_comment']) ?></div>
            <?php endif; ?>
        <?php elseif ($order['return_status'] === 'cancelled'): ?>
            ...
            <?php if (!empty($order['return_cancel_comment'])): ?>
                <div style="margin-top: 5px;">Cancelled Comment: <?= htmlspecialchars($order['return_cancel_comment']) ?></div>
            <?php endif; ?>
        <?php endif; ?>
    <?php endif; ?>
    ...
    <form method="post" action="<?php echo base_url('user/return_order/' . $order['id']) ?>">
        <a href="<?php echo base_url('user/my_orders') ?>" class="back-link">← My Orders</a>
            <a href="<?= base_url('user/return_order/' . $order['id']) ?>" onclick="return confirm('Are you sure you want to return this order?')">
